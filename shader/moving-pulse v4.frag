...
//
const int size_of_display_in_columns = 6;    // how many parallel pulses to run
...
void Initialize_pulse_settings_array(inout Pulse_settings pulse[size_of_display_in_columns]){
    ...
    pulse[0] = Pulse_settings(vec3(0.0, 1.0, 0.0), vec3(0.0, 0.0, 0.05), vec3(0.0, 0.0, 0.0), 5.0, 1.0, 3.0, 0.0, 0.025, 0.0, false, false, true);  // Mouse bottom
    pulse[1] = Pulse_settings(vec3(0.0, 1.0, 0.0), vec3(0.0, 0.0, 0.05), vec3(0.0, 0.0, 0.0), 3.0, 2.0, 3.0, 0.0, 0.025, 0.0, false, false, true);  // Mouse top
    pulse[2] = Pulse_settings(vec3(0.0, 1.0, 0.0), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), 4.0, 1.0, 3.0, 0.0, 0.025, 0.0, false, false, true);  // Graphics Card
    pulse[3] = Pulse_settings(vec3(0.0, 1.0, 0.0), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), 14.0, 1.0, 3.0, 0.0, 0.025, 0.0, false, false, true);  // Case Front
    pulse[4] = Pulse_settings(vec3(0.0, 1.0, 0.0), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), 10.0, 1.0, 3.0, 0.0, 0.025, 0.0, false, false, true);  // Case Top
    pulse[5] = Pulse_settings(vec3(0.0, 1.0, 0.0), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), 4.0, 1.0, 3.0, 0.0, 0.025, 0.0, false, false, true);  // Motherboard
}
//
...
vec3 generate_rgb_output_column(Pulse_settings pulse, float distance_of_current_location_from_pulse_as_percent, float distance_of_current_location_behind_start_of_led_in_percent){
    ...
    if(pulse.display_discrete_led_output){
        ...
        if ( (distance_of_current_location_from_pulse_as_percent + distance_of_current_location_behind_start_of_led_in_percent) > (ratio_of_total_cycle_to_display) ){
            // Black magic.
            ...
            //
            // Essentially: Generate the entire first LED in the pulse as soon as the pulse index enters it's boundaries
            ...
            distance_of_current_location_from_pulse_as_percent = distance_of_current_location_from_pulse_as_percent + (distance_of_current_location_behind_start_of_led_in_percent);
        }
    }
